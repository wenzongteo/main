...
public class AddCommandParserTest {
    ...
    public void parse_optionalFieldsMissing_success() {
        ...
        expectedPerson = new PersonBuilder().withName(VALID_NAME_AMY).withPhone(NOT_FILLED)
                .withEmail(VALID_EMAIL_AMY).withAddress(NOT_FILLED).withTags().build();
        assertParseSuccess(parser, AddCommand.COMMAND_WORD + NAME_DESC_AMY + EMAIL_DESC_AMY,
                new AddCommand(expectedPerson));

        // missing 2 fields (phone and address) - using command alias
        expectedPerson = new PersonBuilder().withName(VALID_NAME_AMY).withPhone(NOT_FILLED)
                .withEmail(VALID_EMAIL_AMY).withAddress(NOT_FILLED).withTags().build();
        assertParseSuccess(parser, AddCommand.COMMAND_ALIAS + NAME_DESC_AMY + EMAIL_DESC_AMY,
                new AddCommand(expectedPerson));
    }
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, AddCommand.COMMAND_WORD + NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB
                        + ADDRESS_DESC_BOB + INVALID_TAG_DESC + VALID_TAG_FRIENDS, Tag.MESSAGE_TAG_CONSTRAINTS);
