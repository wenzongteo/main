...
*Code contributed*: +
Functional Code: [https://github.com/CS2103AUG2017-W14-B2/main/blob/master/collated/main/wenzongteo.md[Main file]] [https://github.com/CS2103AUG2017-W14-B2/main/blob/master/collated/main/wenzongteo-reuse.md[Reused Code]] +
...
The previous implementation only allow an user to add a contact into Augustine when the user inputs data for all of the
attributes of a contact in Augustine. However, it is unlikely that an user will know all the particulars of their contacts.
Therefore, I have implemented the optional fields enhancement onto Augustine so that an user only need need to input
the name and email address of a contact to add the contact into Augustine. The name and email address of each contact are
compulsory as these are the two fields for a contact the user will definitely.
...
identifier for an user, it's hard for the user to associate an email address to a contact. A photo for a contact can
