...
Developed in Java, Augustine will runs on Windows, Mac, and Unix environments, catering to all students in NUS.
...
[IMPORTANT]
We are only concerned with whether the contact's photo and email address are updated. Thus, modification of other
...
====
Problem::
When there are no contacts to be listed, the area where the personListView is suppose to be will display a white box.
====
