...
* `*EmailSend*`: Handles the sending of email by setting up the Simple Mail Transfer Protocol (SMTP) connection for sending email via JavaMail API.
The email is rejected if the `MessageDraft` object in `EmailCompose` is empty of the login details in `EmailLogin` is invalid.
...
up the Simple Mail Transfer Protocol (SMTP) for email delivery.
...
isenforced through the `USERNAME_VALIDATION_REGEX` String implemented in the class, failing which will cause an
`IllegalValueException` to be thrown.
