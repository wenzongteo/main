...
command executed, the Model component may be accessed. (see Figure 6).
...
[caption="Figure 15: "]
.Undo Redo execute undo stack diagram
...
The diagram below shows how the email draft tab in the Augustine User interface (UI) is updated when the `EventCenter` reacts to the `EmailDraftChangedEvent` in Figure 25.
...
The following diagram (Figure 28) highlights the flow of logic when the user inputs the Instagram command:


[caption="Figure 28: "]
.Flow of Instagram command
