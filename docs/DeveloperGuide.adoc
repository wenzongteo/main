...
Figure 3.4.1 below shows a snippet of how a method in `ParserUtil` detects whether is there a value in the `ArgumentMultiMap` of the associated field and if there is no value, it will return a '-' to AddCommandParser
instead.
...
Figure 3.4.2 below shows you the sequence diagram of how the `Add Command` works in Augustine.

[caption="Figure 2.4.2.3 "]
.Sequence Diagram for the Add Command
image::addSequenceDiagram.png[width="200"]
