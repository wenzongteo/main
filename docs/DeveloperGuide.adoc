...
image::addPhotoActivityDiagram.png[width="640"]

===== Implementation Details

As seen in _Figure X_ above, the `AddCommandParser` is used to parse the input entered by the user into the different fields for a contact.

In `AddCommandParser`, Augustine will do the following. +

. Check if the two compulsory prefixes, n/ and t/, which are used for the name and email fields respectively are present.
.. If the two compulsory prefixes are not present, Augustine will throw an error message to the user, stating both the name and email fields
are required to be filled.
. Execute the `checkInput` method in `AddCommandParser` to determine if a field is filled by the user. If the field is not filled,
...
[start = 4]
. Create the `Person` object and call `AddCommand` and pass in the newly created `Person` object as an parameter.
...
`data/images` folder to centralize the storage of the photos. The photos are also renamed to follow a fixed format where the file is named as
...
contacts in Augustine while Augustine is running and enable the restoration of a contact's previous photos when the undo function is executed.
Upon the exit of Augustine, Augustine will delete both the folder and all the photos in the folder.

===== Add
When adding a new contact into Augustine, Augustine will do the following.
...
system so that the photo can be restored if an undo command is executed. Therefore, when the user edits a contact in Augustine, *one* of the
...
Below is a code snippet of the updateCasesForPhoto method in the `UniquePersonList` class which determines which of the 4 cases an edit command
belongs to.
