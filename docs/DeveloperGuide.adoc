...
Augustine is a virtual Address Book that seeks to assist students that are currently pursuing their studies in National
University of Singapore (NUS). As a virtual Address Book, Augustine can provide students with a comprehensive software
to virtually store contact details of their friends in NUS.

Command Line Interface (CLI) is the primary mode of input in Augustine while a General User Interface is used to give
users visual feedback of queries that are performed.

Developed in Java, Augustine can cater to all students as it can be executed in both Windows and Unix environment.

== Purpose [Edited in v1.4]

This Developer Guide seeks to assist both experienced and new Developers that are interested in contributing to Augustine
or understand the underlying mechanisms of Augustine.
...
=== Architecture Design [Edited in v1.3 and v1.4]
...
The UI consists of a `MainWindow` that is made up of smaller parts; `CommandBox`, `ResultDisplay`, `LeftDisplayPanel`, `PersonListPanel`, `MessageDisplay`, `StatusBarFooter`, `BrowserPanel`. All of these parts, including the `MainWindow`, inherit from the abstract `UiPart` class.
...
image::LogicClassDiagram.png[width="800"]
_Figure 4.3.2a : Structure of the Logic Component_

image::LogicCommandClassDiagram.png[width="800"]
_Figure 4.3.2b : Structure of Commands in the Logic Component. This diagram shows finer details concerning `XYZCommand` and `Command` in Figure 4.3.1a_
...
_Figure 4.3.2c : Interactions Inside the Logic Component for the `delete 1` Command_
...
==== Email component [Edited in v1.3 and v1.4]
...
// tag::imagestorage[]
=== Image Storage [Edited in v1.4]
Each contact in Augustine can be appointed with a specific Image so that the User can easily identify a Contact within Augustine.

[NOTE]
A default image will be appointed to the Contact if the User: +
1. not specify an image for the Contact during the Contacts creation
2. if the Contact's image is removed by the User during edit.

The image appointed to a Contact will be copied over to the `data/images` folder and renamed as "contact_email_address.jpg".

When the User edits a Contact in Augustine, *only one* of the 4 possible cases will occur:

[IMPORTANT]
We are only concern if whether the Contact's Display Picture and email address is updated and thus modification of other attributes are not mentioned
in the cases below.
...
If the Contact is deleted from Augustine by the User, the appointed Image for the Contact is also deleted from the computer.

---

==== Design Considerations
**Aspect:** Storing of Contact's Display Picture. +
**Alternative 1 (current choice):** Create a copy of the image that is in the path entered by user into a designated folder in Augustine.
Store the file path of the copied image into the Contact so that future rendering of the Contact's Display Picture will be to the copied file. +
**Pros:** Changing of file path or removal of the original image by the User will not affect the display of Contact's Display Picture in Augustine. +
**Cons:** Addition storage space on the computer is required to create a possibly duplicated copy of an image in the computer.
**Alternative 2:** Store the file path of the original image entered by the User into the Contact and future rendering of the Contact's Display Picture
will be from the original Image +
**Pros:** Additional storage space on the computer is not required.
**Cons:** Changing of file path or removal of the image will lead to invalid display of a Contact's Display Picture. +
...
Each improvement is phrased in a problem-solution format.
...
Solution::
When verifying if a module code is correct, check against a list of modules from either IVLE or NUSmods API instead of using a regular expression.
====
...
Being a recent addition to Augustine, the email feature is not fully fledged. The following are possible improvements which can improve the process of emailing in Augustine:
