...
** For example, if you see `add n/NAME`, `NAME` is a compulsory parameter supplied by you.
** E.g.  `add n/John Doe`
...
* Words in `angled brackets` < > separated by a bar `|` requires you to fill up one of the parameters.
...
* The command edits the person of the specified `INDEX`.
...
* In _Figure 6_, Bernice had a "colleagues" tag before the command was run.
* Since Bernice was already tagged under "colleagues", this command removes this tag from Bernice.
* Since Bernice was not already tagged under "friends", this command adds the "friends" tag to Bernice.
* _Figure 7_ shows the result after the command is run.
...
* In _Figure 8_, Bernice's phone number is 99272758, and her email is berniceyu@example.com
* The command will change the phone number to 87921929 and email to bernice@example.com
* _Figure 9_ shows the result after the command is ran.
...
+
--
You can find contacts and sort the resulting contact list displayed by name, tag, email or address. Type the command below to
...
+
--
You can [fuchsia]`filter` contacts by multiple name. The command below will display the
...
+
--
You can [fuchsia]`filter` contacts by tags. The command below will display only the contacts tagged under "Owes money"
...
*A*: Install the application in that other computer and overwrite the empty data file generated with your original data.
