...
The command below deletes the second person on the list (see Figure 11).
...
[caption="Figure 11. "]
.Before and after results of deleting second person on the list
...
the `find` command (see Figure 12).
...
[caption="Figure 12. "]
.Before and after results of deleting first person on the list
...
* This command requires the *message* (em/), *subject* (es/), *Gmail address/password* (el/) fields to be filled before an email can be send out.
...
// tag::backup[]
=== Backing up Augustine data to a separate file: [fuchsia]`backup`
