...
+
--
You cannot add a new contact that has the same email address as another contact in Augustine (see Figure 5).
...
Augustine data is saved to your hard disk automatically whenever a command changes it. +
...
(located at `/data/addressbook.xml` from the program's root directory) with the same file from your original computer.
